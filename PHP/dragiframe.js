...
function addHandle(o, win) 
{
	...
	{
		if(typeof(window.opera)!='unknown')
		{
			win.eval("document.onmousedown = function (evt) { if(typeof(evt)=='undefined') evt = event; "+topRefStr+".parent.DIF_begindrag(evt, "+topRefStr+"); };");
			win.eval("document.onmouseup = function (evt) { if(typeof(evt)=='undefined') evt = event; "+topRefStr+".parent.DIF_enddrag(evt, "+topRefStr+"); };");
			win.eval("document.onmousemove = function (evt) { if(typeof(evt)=='undefined') evt = event; "+topRefStr+".parent.DIF_iframemove(evt, "+topRefStr+"); };");
		...
		else
		{
			...
			win.eval("function OnMouseDownHandler(evt) { if(typeof(evt)=='undefined') evt = event; "+topRefStr+".parent.DIF_begindrag(evt, "+topRefStr+"); }");
			win.eval("document.onMouseDown = OnMouseDownHandler;");
			win.eval("function OnMouseUpHandler(evt) { if(typeof(evt)=='undefined') evt = event; "+topRefStr+".parent.DIF_enddrag(evt, "+topRefStr+"); }");
			win.eval("document.onMouseUp = OnMouseUpHandler;");
			win.eval("function OnMouseMoveHandler(evt) { if(typeof(evt)=='undefined') evt = event; "+topRefStr+".parent.DIF_iframemove(evt, "+topRefStr+"); }");
			win.eval("document.onMouseMove = OnMouseMoveHandler;");
			win.DIF_handlersAdded = true;
			win.DIF_mainHandlersAdded = true;
		}
	}
	...
	{
		window.eval("function OnMouseMoveHandler2(evt) { if(typeof(evt)=='undefined') evt = event; DIF_mouseMove(evt, window); }");
		window.eval("document.onMouseMove = OnMouseMoveHandler2;");
...
function isHandleClicked(handle, objectClicked) 
{
	...
	{
		if (objectClicked==handle) return true;
...
function DIF_drag(x,y) 
{
  ...
  else if( document.body && ( document.body.clientWidth || document.body.clientHeight ) ) 
  {
	...
	{
		if (newPositionX > document.body.scrollLeft && newPositionX + parseInt(DIF_iframeObjects[DIF_iframeBeingDragged].offsetWidth,10) - document.body.scrollLeft < myW)
		{
			DIF_iframeObjects[DIF_iframeBeingDragged].style.left = newPositionX + "px";
			DIF_pageMouseDownLeft[DIF_iframeBeingDragged] += x;
		}
		if (newPositionY > document.body.scrollTop && newPositionY + parseInt(DIF_iframeObjects[DIF_iframeBeingDragged].offsetHeight,10) - document.body.scrollTop < myH) 
		{
